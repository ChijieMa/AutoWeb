...
controller.prototype.setCookie = function(cookieObj) {
    ...
    this.cookieObj[cookieObj.key] = this.res.formatCookie(cookieObj);
    var cookieArr = [];
    for (var key in this.cookieObj) {
        cookieArr.push(this.cookieObj[key]);
    }
    this.res.setCookie(cookieArr);
};

controller.prototype.clearCookie = function(name) {
    var obj = {
        key: String(name),
        value: '',
        expires: new Date(0)
